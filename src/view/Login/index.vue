<template>
    <div class="login_page unselectable">
        <div class="login_container">
            <div v-if="showLogin" class="login_box animate">
                <div class="title">欢迎登录</div>
                    ...
                    <input v-model="loginForm.password" type="password" placeholder="密码" @blur="passwordCheck(loginForm.password)">
                    <p class="error">{{check_error.password_error}}</p>
                    <el-button @click="login">登录</el-button>p
                <div class="to">没有账号？<span class="ok" @click="toWhere(false)">去注册</span></div>
            </div>
            <div v-else class="sign_box animate">
                <div class="title">欢迎注册</div>

                       <input v-model="signForm.username" type="text" placeholder="用户名" @blur="sign_usernameCheck(signForm.username)">

                       <p class="error">{{ check_error.sign_username_error }}</p>

                       <input v-model="signForm.password" type="password" placeholder="密码" @blur="sign_passwordCheck(signForm.password)">

                       <p class="error">{{check_error.sign_password_error}}</p>

                       <input v-model="signForm.password2" type="password" placeholder="重复密码" @blur="password2Check(signForm.password2)">
                       ...
                       <div class="code_box">
                            <input v-model="signForm.code" class="code" type="text" placeholder="验证码" @blur="codeCheck(signForm.code)">
...
<style lang="less" scoped>
    ...
    .login_page{
        width: 100vw;
        height: 100vh;
        background-image: url('../../assets/img/lg_bg.jpg');
